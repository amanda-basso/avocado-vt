...
def remote_commander(client, host, port, username, password, prompt,
    ...
    :raise remote.LoginBadClientError: If an unknown client is requested
    :raise: Whatever remote.handle_prompts() raises
    ...
    outw = remote.AexpectIOWrapperOut(session)
    # Create commander
...
def run_remote_cmd(cmd, params, remote_runner=None, ignore_status=True):
    ...
    try:
        ...
        return cmdresult
            ...
            remote.LoginAuthenticationError, remote.LoginProcessTerminatedError) as e:
        logging.error(e)
...
class Remote_Package(object):
    ...
    def pull_file(self, local_path, timeout=600):
        ...
        logging.debug("Pull remote: '%s' to local: '%s'." % (self.remote_path,
                                                             local_path))
        remote.copy_files_from(self.address, self.cp_client, self.username,
    ...
    def push_file(self, local_path, timeout=600):
        ...
        logging.debug("Push local: '%s' to remote: '%s'." % (local_path,
                                                             self.remote_path))
        remote.copy_files_to(self.address, self.cp_client, self.username,
...
class RemoteFile(object):
    ...
    def __init__(self, address, client, username, password, port,
        ...
        try:
            self._pull_file()
        except remote.SCPTransferFailedError:
    ...
    def _pull_file(self):
        ...
        if self.client == "test":
            ...
            remote.copy_files_from(self.address, self.client, self.username,
...
class RemoteRunner(object):
    ...
    def __init__(self, client="ssh", host=None, port="22", username="root",
        ...
        if session is None:
            ...
            self.session = remote.wait_for_login(client, host, port, username,
